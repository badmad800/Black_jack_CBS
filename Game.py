...
class Game:
    ...
    def check_winner(self):
        if self.dealer.full_points > 21:
            #  all win
            print('Dealer are fall ! All players in game  win !')
            ...
            for player in self.players:
                if player.full_points == self.dealer.full_points:
                    ...
                    print(MESSAGE.get('eq').format(player= player, points= player.full_points))
                elif player.full_points > self.dealer.full_points:
                    player.money +=player.bet * 2
                    if isinstance(player, Player.Bot):
                        print(MESSAGE.get('win').format(player))
                    elif isinstance(player,Player.Player):
                        print(MESSAGE.get('You are win !'))

                elif player.full_points < self.dealer.full_points:
                    if isinstance(player, Player.Bot):
                        print(MESSAGE.get('lose').format(player))
                    elif isinstance(player,Player.Player):
                        print(MESSAGE.get('You are lose !'))
    ...
    def start_game(self):
        ...
        while True
            ...
            # print player card after first deal
            ...
            if not self._ask_starting(MESSAGE.get('rerun')):
                break
